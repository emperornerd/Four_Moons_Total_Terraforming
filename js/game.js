...
// Optional dev/test override. Normally null. When a misc/dev.html launcher
// page loads ../index.html inside an iframe and posts a message, this gets
// filled with the desired starting state. initGame() applies it once and
// clears it, so a normal game start is completely unaffected.
let DEV_SETUP=null;
let DEV_START_TILE=null; // {x,y} dev override start tile (consumed at player placement)
...
function openBuildingPanel(b){
  ...
  for(let a of b.actions){
    ...
    btn.className='pbtn';
    let canAfford=!usedToday&&!soldHere&&!disabledReason&&ap>=a.ap&&canPayResourceCost(a.cost);
    btn.disabled=!canAfford;
    let costDisplay=displayCost((typeof a.costFn==='function')?a.costFn():(a.cost));
    btn.innerHTML=(a.label+(soldHere?' <span class="cost">SOLD</span>':usedToday?'<br><span class="flavor">Done for today.</span>':disabledReason?'<br><span class="flavor">'+disabledReason+'</span>':' <span class="cost">'+a.ap+' AP'+(costDisplay&&costDisplay!=='free'&&costDisplay!=='$0'?' + '+costDisplay:'')+'</span>')+(a.flavor?'<br><span class="flavor">'+a.flavor+'</span>':''));
    btn.onclick=()=>{
...
function makeFullActionButton(label,cost,apCost,core,n){
  ...
  let canAfford=ap>=apCost&&canPayResourceCost(cost);
  btn.disabled=!canAfford;
...
function openNPCPanel(n){
  ...
  qb.className='pbtn';
  qb.innerHTML=quickName+' <span class="cost">0 AP</span><br><span class="flavor">Free quick touch, small effect</span>';
  qb.onclick=()=>{
...
function triggerEvent(){
  ...
  for(let c of ev.choices){
    ...
    btn.disabled=!canAfford;
    btn.innerHTML=c.label+' <span class="cost">'+c.ap+' AP'+(c.cost?' + '+displayCost(c.cost):'')+'</span>';
    btn.onclick=()=>{
...
function initGame(fromSnap){
  ...
  resetDailyInteractions();
  // ---- DEV/TEST OVERRIDE --------------------------------------------------
  // If a dev setup is present (set via misc/dev.html through a ?dev=/#dev=
  // URL payload, postMessage, or the __DEV_START__ API), apply it now so the
  // game boots into the exact state chosen (stats, phase, AP, Gargoyliani
  // bills, sold assets, legal issues, day, start position). Nothing here runs
  // on a normal page load (DEV_SETUP stays null), so normal play is untouched.
  if(!DEV_SETUP)DEV_SETUP=readDevPayloadFromURL();
  if(DEV_SETUP){
    const d=DEV_SETUP;
    ...
    if(d.S&&typeof d.S==='object'){
      const statKeys=['credits','approval','fear','loyalty','military','institutions','population','media'];
      for(const k of statKeys){
        const v=num(d.S[k],null);
        if(v!=null)S[k]=clamp(v,LOSE.bankruptcyClamp,STAT_MAXES[k]);
      }
    }
    if(d.phase!=null&&!isNaN(Number(d.phase)))phase=clamp(Number(d.phase),0,PHASES.length-1);
    const dc=num(d.dayCount,dayCount);if(dc!=null)dayCount=Math.max(1,Math.round(dc));
    const apv=num(d.ap,null);if(apv!=null)ap=clamp(apv,0,999);
    const gv=num(d.gargoyle,null);if(gv!=null)gargoyleBills=clamp(gv,0,gargoyleMax);
    if(d.soldAssets&&Array.isArray(d.soldAssets)){
      soldAssets={};
      for(const a of CARVE_UP_ASSETS)soldAssets[a]=d.soldAssets.includes(a);
    }
    if(d.legalCount!=null&&!isNaN(Number(d.legalCount))){
      const n=Math.max(0,Math.min(10,Math.round(Number(d.legalCount))));
      legalIssues=[];
      for(let i=0;i<n;i++)spawnLegal();
    }
    // Custom start position (tiles). Applied at player placement below.
    if(d.startTile&&typeof d.startTile==='object'&&!isNaN(Number(d.startTile.x))&&!isNaN(Number(d.startTile.y))){
      DEV_START_TILE={
        x:Math.max(1,Math.min(MAP_W-2,Math.round(Number(d.startTile.x)))),
        y:Math.max(1,Math.min(MAP_H-2,Math.round(Number(d.startTile.y)))),
      };
    }else{
      DEV_START_TILE=null;
    }
    DEV_SETUP=null; // consumed; a reload/retry starts normally again
  }
  // -------------------------------------------------------------------------
  ...
  if(DEV_START_TILE){player.x=DEV_START_TILE.x*TILE;player.y=DEV_START_TILE.y*TILE;DEV_START_TILE=null;}
...
// If the page was opened with a dev payload (?dev= / #dev=), auto-start the
// game once the DOM is up so misc/dev.html "just runs" it. Normal loads skip
// this entirely (no payload).
if(readDevPayloadFromURL()){
  setTimeout(function(){ try{ beginGame(); }catch(e){ console.error('dev autostart failed:',e); } }, 0);
}

// ============================================================
// DEV/TEST LAUNCHER HOOK
// A misc/dev.html launcher can load this page inside an iframe
// and post a setup message BEFORE the player clicks BEGIN, so the
// game boots into an exact chosen state for testing. A normal page
// load never receives such a message, so the main game is untouched.
// The message must be an object with a `__devSetup` token and a
// numeric `S` map, phase, ap, etc. Non-numeric/garbage values are
// ignored (initGame's consumer only reads well-formed numbers).
// ============================================================
window.addEventListener('message',(e)=>{
  const d=e&&e.data;
  if(!d||!d.__devSetup)return;               // ignore everything except our own token
  if(!gameStarted)return;                    // only before the game boots (still on title screen)
  DEV_SETUP=sanitizeDevSetup(d);             // consumer re-clamps/validates anyway
});

// Synchronous, race-free entry used by the same-origin misc/dev.html
// launcher: set the setup immediately and then beginGame() normally.
// Exposed globally so the launcher iframe's parent can call it directly.
function sanitizeDevSetup(d){
  if(!d||typeof d!=='object')return null;
  const s=(d.S&&typeof d.S==='object')?(()=>{const out={};for(const k of ['credits','approval','fear','loyalty','military','institutions','population','media']){if(d.S[k]!==undefined)out[k]=Number(d.S[k]);}return out;})():null;
  return {
    S:s,
    phase:(d.phase!==undefined)?Number(d.phase):undefined,
    dayCount:(d.dayCount!==undefined)?Number(d.dayCount):undefined,
    ap:(d.ap!==undefined)?Number(d.ap):undefined,
    gargoyle:(d.gargoyle!==undefined)?Number(d.gargoyle):undefined,
    soldAssets:Array.isArray(d.soldAssets)?d.soldAssets.slice():undefined,
    legalCount:(d.legalCount!==undefined)?Number(d.legalCount):undefined,
    startTile:(d.startTile&&typeof d.startTile==='object'&&d.startTile.x!==undefined&&d.startTile.y!==undefined)?{x:Number(d.startTile.x),y:Number(d.startTile.y)}:undefined,
  };
}
window.__DEV_START__=function(setup){DEV_SETUP=sanitizeDevSetup(setup);};
window.__DEV_CLEAR__=function(){DEV_SETUP=null;DEV_START_TILE=null;};

// ---------------------------------------------------------------------------
// DEV/TEST LAUNCHER (URL handoff)
// A misc/dev.html launcher can load this page as ".../index.html?dev=JSON"
// (or "#dev=JSON"). The game reads the payload at boot, applies it to the
// starting state, and auto-starts — so no cross-frame calls are needed and
// it works over any http(s) host (and even file://). A normal page load has
// no "dev" parameter, so the main game is completely untouched.
// ---------------------------------------------------------------------------
function readDevPayloadFromURL(){
  let raw=null;
  const sp=new URLSearchParams(location.search);
  if(sp.has('dev'))raw=sp.get('dev');               // already URL-decoded by URLSearchParams
  else{
    // Hash fallback: location.hash is not auto-decoded, so decode exactly once.
    const h=location.hash||'';
    const i=h.indexOf('dev=');
    if(i!==-1)raw=h.slice(i+4);
    if(raw!=null){ try{ raw=decodeURIComponent(raw); }catch(e){} }
  }
  if(!raw)return null;
  try{
    return sanitizeDevSetup(JSON.parse(raw));
  }catch(e){
    return null;
  }
}
